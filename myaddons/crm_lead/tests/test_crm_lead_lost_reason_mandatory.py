...
class TestCrmLeadLostReasonMandatory(TransactionCase):
    ...
    def test_action_set_lost_without_reason_blocked(self):
        lead = self._create_lead()

        with self.assertRaises(UserError):
            lead.action_set_lost()

    def test_action_set_lost_with_reason_succeeds(self):
        reason = self.env["crm.lost.reason"].create({"name": "Test Reason"})
        lead = self._create_lead()
